...
int InitDCMem(SimulationStruct *sim, cl_context context, cl_command_queue command_queue, cl_mem *simparam_floats_mem_obj,cl_mem *simparam_ints_mem_obj, cl_mem *layerspecs_mem_obj )
{
  ...
  float h_simparam_floats[3];
  UINT32 h_simparam_ints[4];
  ...
  h_simparam_floats[0] = sim->start_weight;
  h_simparam_floats[1] = sim->det.dz;
  h_simparam_floats[2] = sim->det.dr;
  
  h_simparam.na = sim->det.na;
  h_simparam.nz = sim->det.nz;
  h_simparam.nr = sim->det.nr;

  h_simparam_ints[0] = sim->det.na;
  h_simparam_ints[1] = sim->det.nz;
  h_simparam_ints[2] = sim->det.nr;
  h_simparam_ints[3] = sim->n_layers;
  ...
  //  exit(-1);
  //}
  *simparam_floats_mem_obj = clCreateBuffer(context, CL_MEM_READ_ONLY, sizeof(float) * 3, NULL, &ret);
  if(ret!= CL_SUCCESS){
    printf("Error creating simparam buffer, exiting\n");
    exit(-1);
  }
  *simparam_ints_mem_obj = clCreateBuffer(context, CL_MEM_READ_ONLY, sizeof(UINT32) * 4, NULL, &ret);
  if(ret!= CL_SUCCESS){
    printf("Error creating simparam buffer, exiting\n");
    exit(-1);
  }
  //ret = clEnqueueWriteBuffer(command_queue, *simparam_mem_obj, CL_TRUE, 0, sizeof(SimParamGPU), &h_simparam, 0, NULL, NULL); 
  ret = clEnqueueWriteBuffer(command_queue, *simparam_floats_mem_obj, CL_TRUE, 0, sizeof(float) * 3, h_simparam_floats, 0, NULL, NULL); 
  ret = clEnqueueWriteBuffer(command_queue, *simparam_ints_mem_obj, CL_TRUE, 0, sizeof(UINT32) * 4, h_simparam_ints, 0, NULL, NULL); 
  //ret = clEnqueueWriteBuffer(command_queue, *simparam_mem_obj, CL_TRUE, 0, sizeof(SimParamGPU), &h_simparam, 0, NULL, NULL); 
  ...
  for (UINT32 i = 0; i < n_layers; ++i)
  {
    ...
    h_layerspecs_floats[i*9] =sim->layers[i].z_min;
    h_layerspecs[i].z1 = sim->layers[i].z_max;
    h_layerspecs_floats[i*9 + 1] =sim->layers[i].z_max;
    float n1 = sim->layers[i].n;
    h_layerspecs[i].n = n1;
    h_layerspecs_floats[i*9 + 2] = n1;

    // TODO: sim->layer should not do any pre-computation.
    float rmuas = sim->layers[i].mutr;
    h_layerspecs[i].muas = FP_ONE / rmuas;
    h_layerspecs_floats[i*9 + 3] = FP_ONE / rmuas;
    h_layerspecs[i].rmuas = rmuas;
    h_layerspecs_floats[i*9 + 4] = rmuas;
    h_layerspecs[i].mua_muas = sim->layers[i].mua * rmuas;
    h_layerspecs_floats[i*9 + 5] =sim->layers[i].mua * rmuas;

    h_layerspecs[i].g = sim->layers[i].g;
    h_layerspecs_floats[i*9 + 6] =sim->layers[i].g;

    if (i == 0 || i == n_layers-1)
    {
      h_layerspecs[i].cos_crit0 = MCML_FP_ZERO;
      h_layerspecs_floats[i*9 + 7] = MCML_FP_ZERO;
      h_layerspecs[i].cos_crit1 = MCML_FP_ZERO;
      h_layerspecs_floats[i*9 + 8] = MCML_FP_ZERO;
    }
    else
    {
  ...
  *layerspecs_mem_obj = clCreateBuffer(context, CL_MEM_READ_ONLY, sizeof(float) * 9 * n_layers, NULL, &ret);
  ...
  //ret = clEnqueueWriteBuffer(command_queue, *layerspecs_mem_obj, CL_TRUE, 0, sizeof(float) * 9 *n_layers, h_layerspecs_floats, 0, NULL, NULL); 
        ...
        )
{
  ...
  //ret = clEnqueueWriteBuffer(command_queue, *num_photons_simulated_mem_obj, CL_TRUE, 0, size, HostMem->n_photons_left, 0, NULL, NULL); 
  //if(ret!= CL_SUCCESS){
  ...
  size = NUM_THREADS * sizeof(UINT32);
  *num_photons_simulated_mem_obj = clCreateBuffer(context, CL_MEM_READ_WRITE, size, NULL, &ret); 
  if(ret!= CL_SUCCESS){
...
void FreeDeviceSimStates(cl_context context, cl_command_queue command_queue, cl_kernel kernel, 
        cl_program program, cl_mem simparam_floats_mem_obj, cl_mem simparam_ints_mem_obj,  cl_mem layerspecs_mem_obj, cl_mem num_photons_simulated_mem_obj, 
     ...
     )
{
 ...
 ret = clReleaseMemObject(simparam_floats_mem_obj);
 if(ret!= CL_SUCCESS){
    printf("Error releasing simparam mem obj, exiting\n");
    exit(-1);
 }
 ret = clReleaseMemObject(simparam_ints_mem_obj);
 if(ret!= CL_SUCCESS){
    printf("Error releasing simparam mem obj, exiting\n");
    exit(-1);
 }
 ...
 ret = clReleaseMemObject(num_photons_simulated_mem_obj);
 if(ret!= CL_SUCCESS){
