...
// TODO: Optimization: Remove members that aren't necessary for every stage
...
)
{
	...
	while(!done) {
		...
		if(!hopper.bubble && !hopper_stall) {
			Material mat = materials[hopper.tetra.matID];		
			...
			float travel_distance = hopper.step * mat.rmu_as;
			...
			hopper.step -= iface_distance * mat.mu_as;
		}

		// Hopper2 work (should be identical to hopper1)
		if(!hopper2.bubble && !hopper2_stall) {
			Material mat = materials[hopper2.tetra.matID];
			...
			float travel_distance = hopper2.step * mat.rmu_as;
			...
			// Always subtract from step. This is correct if it hops.
			// If it doesn't hop a new step will simply be drawn
			hopper2.step -= iface_distance * mat.mu_as;
		}

		// Dropper work
		if(!dropper.bubble && !dropper_stall) {
			Material mat = materials[dropper.tetra.matID];

			float dw = dropper.pkt.weight * mat.absfrac;
		...
		if(!spinner.bubble && !spinner_stall) {
			Material mat = materials[spinner.tetra.matID];

			spinner_rng = unit_rand(spinner_rng.seed);
			Spinner spinner_t = spin(spinner.pkt, mat, spinner_rng.rand);
		...
		if(!reflactor.bubble && !reflactor_stall) {
			Material mat = materials[reflactor.tetra.matID];
		...
		done = launcher.bubble && stepper.bubble && hopper.bubble && hopper2.bubble && reflactor.bubble && looker.bubble && dropper.bubble && spinner.bubble;

#ifdef DEBUG
		printf("%d-%d-%d %d-o\n", launcher.payload_id, stepper.payload_id, hopper.payload_id, hopper2.payload_id);
		printf("  | |X| |\n");
