...
set base_sof_filename "base.sof"
set base_cof_filename "base.cof"
set base_pof_filename "base.pof"
set base_map_filename "base.map"
set base_pof_extracted_filename "base_extracted.pof"
...
if { [string equal $revision_name $base_revision_name] }  {
  ...
  if { [file exists $base_sof_filename] } {
	  if {[catch {execute_module -tool cpf -args "-c base.cof"} result]} {
	  post_message -type error "Error generating CvP files - second stage! $result"
	  exit 2
	}
	set OS [lindex $tcl_platform(os) 0]
	if { $OS == "Windows" } {
		if { [catch { exec scripts/bin/pof_extractor.exe $base_pof_filename $base_map_filename $base_pof_extracted_filename} msg] } {
		post_message -type error "Can't run pof_extractor"
		}
	} else {
		if { [catch { exec scripts/bin/pof_extractor -p $base_pof_filename -m $base_map_filename -o $base_pof_extracted_filename} msg] } {
		post_message -type error "Can't run pof_extractor"
		}
	}

    # Add base revision RBF to output fpga.bin file
    if {[catch {qexec "aocl binedit $outfile add .acl.base_revision.rbf $base_pof_extracted_filename"} res]} {
      post_message "$prog: Can't add base revision RBF file $base_pof_extracted_filename to $outfile: $res"
      exit 2
    }
  } else {
    post_message -type error "Couldn't find base compile SOF in current directory: $base_sof_filename"
    exit 2
  }
