...
if {[catch {execute_module -tool cpf -args "-c --cvp $revision_name.sof $revision_name.rbf"} result]} {
...
if {[catch {set sdk_root $::env(ALTERAOCLSDKROOT)} result]} {
  ...
  if {[file exists $revision_name.periph.rbf]} {
    if {[catch {qexec "aocl hash $revision_name.periph.rbf > $revision_name.periph_hash"} res]} {
      post_message "$prog: Can't hash periphery RBF file : $revision_name.periph.rbf : $res"
    }
  }
