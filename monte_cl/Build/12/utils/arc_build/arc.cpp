...
int main(int argc, char** argv, char** envp)
{
	...
	if(strcmp(argv[1], "shell") == 0) {
		...
		if(argc != 3) {
			...
			strcpy(monte_build, "MONTE_BUILD=Trunk");
			...
			// TODO: Check if build_num exists
			if(strlen(build_num) == 0) {
				...
				strcpy(monte_build, "MONTE_BUILD=");
				strcpy(monte_build + strlen(monte_build), build_num);
		...
		char* read_monte_build = getenv("MONTE_BUILD");
		if(read_monte_build == NULL) {
			printf("Error: MONTE_BUILD environment variable not set!\n");
			exit(0);
		}

		printf("Monte Build %s is currently loaded in the shell\n", read_monte_build);
		exit(0);
	}
	else {
		printf("Uncrecognized arguments\n");
		exit(0);
	}
