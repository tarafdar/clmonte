...
void PopulateTetraFromMeshFile(const char* filename, Tetra **p_tetra_mesh, Point **p_points, TriNode **p_trinodes, TetraNode **p_tetranodes, int *p_Np, int *p_Nt)
{
  ...
  *p_tetranodes = (TetraNode*)malloc(sizeof(TetraNode)*(*p_Nt+1));
  ...
  delete[] faceToTetraMap;
	...
	// Post process add adj mat ID so algorithm can avoid tetra lookup and directly get 
	// neighbouring material interface index coefficient in the case of reflection
	UINT32 z = 1;
	for(z = 1; z <= *p_Np;z++) {
		(*p_tetra_mesh)[z].adjMats[0] = (*p_tetra_mesh)[(*p_tetra_mesh)[z].adjTetras[0]].matID;
		(*p_tetra_mesh)[z].adjMats[1] = (*p_tetra_mesh)[(*p_tetra_mesh)[z].adjTetras[1]].matID;
		(*p_tetra_mesh)[z].adjMats[2] = (*p_tetra_mesh)[(*p_tetra_mesh)[z].adjTetras[2]].matID;
		(*p_tetra_mesh)[z].adjMats[3] = (*p_tetra_mesh)[(*p_tetra_mesh)[z].adjTetras[3]].matID;
	}
  return;
}
