...
__kernel void entry(__global const UINT32* restrict photon_pool, __global const Source* restrict source, __global const Material* restrict materials, __global const Tetra* restrict mesh, __global const UINT32* restrict num_tetras, __global const UINT32* restrict num_mats, __global UINT32* restrict absorption, __global UINT32* restrict transmittance) {
	...
	if(hopper.bubble) hopper.payload_id = 0;
	else if(!hopper_stall) {printf("TID: %d PID %d\n", hopper.pkt.current_tid, hopper.payload_id);
		...
		if(hopper.pkt.dir.x*hopper.pkt.dir.x + hopper.pkt.dir.y*hopper.pkt.dir.y + hopper.pkt.dir.z*hopper.pkt.dir.z > 1.0) break;
		...
		printf("Hopper deduct %f step %f\n", iface_distance, hopper.step);
	}
...
printf("reflect_dir %f %f %f\n", reflactor.pkt.dir.x, reflactor.pkt.dir.y, reflactor.pkt.dir.z);
	...
	printf("FPGA to Earth...Simulation Complete!\n");
