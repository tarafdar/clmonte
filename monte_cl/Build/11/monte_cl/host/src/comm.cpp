...
void init_device_data_seg(cl_context context, cl_command_queue command_queue, UINT32 num_photons, FPGA::Source* source, Tetra* mesh, Material* mats, UINT32* absorption, UINT32* transmittance, DevDataSeg* dev_dseg, unsigned int num_tetras, unsigned int num_mats, UINT32 launcher_rng_init, UINT32 stepper_rng_init, UINT32 dropper_rng_init, UINT32 spinner_rng_init, UINT32 reflactor_rng_init)
{
	...
	write_to_device(context, command_queue, CL_MEM_READ_ONLY, sizeof(UINT32), &launcher_rng_init, &(dev_dseg->launcher_rng_init));
	write_to_device(context, command_queue, CL_MEM_READ_ONLY, sizeof(UINT32), &stepper_rng_init, &(dev_dseg->stepper_rng_init));
	write_to_device(context, command_queue, CL_MEM_READ_ONLY, sizeof(UINT32), &dropper_rng_init, &(dev_dseg->dropper_rng_init));	
	write_to_device(context, command_queue, CL_MEM_READ_ONLY, sizeof(UINT32), &spinner_rng_init, &(dev_dseg->spinner_rng_init));
	write_to_device(context, command_queue, CL_MEM_READ_ONLY, sizeof(UINT32), &reflactor_rng_init, &(dev_dseg->reflactor_rng_init));
}
...
void set_kernel_args(cl_kernel* kernel, DevDataSeg* dev_dseg, UINT32 num_tetras, UINT32 num_mats)
{
	...
	set_kernel_arg(kernel, argnum++, sizeof(cl_mem), &dev_dseg->launcher_rng_init);
	set_kernel_arg(kernel, argnum++, sizeof(cl_mem), &dev_dseg->stepper_rng_init);
	set_kernel_arg(kernel, argnum++, sizeof(cl_mem), &dev_dseg->dropper_rng_init);
	set_kernel_arg(kernel, argnum++, sizeof(cl_mem), &dev_dseg->spinner_rng_init);
	set_kernel_arg(kernel, argnum++, sizeof(cl_mem), &dev_dseg->reflactor_rng_init);
	if(debug_mode) {
		set_kernel_arg(kernel, argnum++, MAX_LOCAL_MEM_SIZE, NULL);
	}
	//set_kernel_arg(kernel, argnum++, sizeof(UINT32)*(/*num_tetras + */1), NULL);
	//set_kernel_arg(kernel, argnum++, sizeof(Material)*num_mats, NULL);
}
