...
__kernel void entry(__global const UINT32* restrict photon_pool, __global const Source* restrict source, __global const Material* restrict materials, __global const Tetra* restrict mesh, __global const UINT32* restrict num_tetras, __global const UINT32* restrict num_mats, __global UINT32* restrict absorption, __global UINT32* restrict transmittance) {
	...
	// Spinner Pipelining
	printf("Spinner.bubble %d, spin2step_q.full %d\n", spinner.bubble, spin2step_q.full);
	...
	// Launcher work
	if(!launcher_stall) {
		launcher.payload_id = launcher_photon_pool;
		launcher.bubble = launcher_photon_pool == 0;
	...
	if(hopper.bubble) hopper.payload_id = 0;
		...
		//if(ifacer.index == 5) break;
		//if(hopper.pkt.dir.x*hopper.pkt.dir.x + hopper.pkt.dir.y*hopper.pkt.dir.y + hopper.pkt.dir.z*hopper.pkt.dir.z > 1.01) break;
	...
	if(dropper.bubble) dropper.payload_id = 0;
		...
		if(dropper.bubble) printf("Packet %d died\n", dropper.payload_id);
	...
	if(matter.bubble) matter.payload_id = 0;
		...
		// TODO: corner case what if this adjacent material is the 0 tetra and doesn't exist?
	...
	if(launcher_push) {
		q_push(&launch2step_q, launcher);
		launcher.bubble = true;
	}

	bool stepper_push = !stepper.bubble && !step2hop_q.full;
	if(stepper_push) {
		q_push(&step2hop_q, stepper);
		stepper.bubble = true;
	}
	...
	if(hopper_push) {
		q_push(hopper2q, hopper);
		hopper.bubble = true;
	}

	bool dropper_push = !dropper.bubble && !drop2spin_q.full;
	if(dropper_push) {
		q_push(&drop2spin_q, dropper);
		dropper.bubble = true;
	}
printf("Spinner.bubble %d, spin2step_q.full %d\n", spinner.bubble, spin2step_q.full);
	bool spinner_push = !spinner.bubble && !spin2step_q.full;
	if(spinner_push) {
		q_push(&spin2step_q, spinner);printf("Spinner Push: %d\n", spinner.payload_id);
		spinner.bubble = true;
	}

	bool matter_push = !matter.bubble && !mat2ref_q.full;
	if(matter_push) {
		q_push(&mat2ref_q, matter);
		matter.bubble = true;
	}

	bool reflactor_push = !reflactor.bubble && !ref2look_q.full;
	if(reflactor_push) {
		q_push(&ref2look_q, reflactor);
		reflactor.bubble = true;
	}

	bool looker_push = !looker.bubble && !look2hop_q.full;
	if(looker_push) {
		q_push(&look2hop_q, looker);
		looker.bubble = true;
	}

	done = launcher.bubble && stepper.bubble && hopper.bubble && matter.bubble && reflactor.bubble && looker.bubble && dropper.bubble && spinner.bubble && launch2step_q.size == 0 && step2hop_q.size == 0 && hop2drop_q.size == 0 && drop2spin_q.size == 0 && spin2step_q.size == 0 && hop2mat_q.size == 0 && mat2ref_q.size == 0 && ref2look_q.size == 0 && look2hop_q.size == 0;
	...
	printf("spin2step_q %d\n", spin2step_q.payload.payload_id);
