...
proc solve_pll_for_vco {output_clock_frequency} {
  ...
  if { $ref_fmax < 1.0 || $ref_fmax > 2000.0} {
    ...
    post_message "Reference clock is: $ref_fmax MHz"
  }
...
set m9k       [get_fitter_resource_usage -resource "M20K*"]
...
if {$do_sta} {
    ...
    puts $sdc "  create_clock -name kernel_clk -period [expr {double(1000) / $k_achieved}] $k_clk_name_full"
    puts $sdc "  create_clock -name kernel_clk2x -period [expr {double(1000) / [expr $k_achieved * 2]}] $k_clk2x_name_full"
    puts $sdc "}"
    close $sdc
