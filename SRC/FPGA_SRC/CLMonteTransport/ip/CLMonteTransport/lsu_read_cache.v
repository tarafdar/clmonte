...
localparam FIFO_DEPTH=ACL_BUFFER_ALIGNMENT/MWIDTH_BYTES;  
localparam FIFO_DEPTH_LOG2=$clog2(FIFO_DEPTH);
localparam CACHESIZE=ACL_BUFFER_ALIGNMENT;
localparam CACHE_ADDRBITS=MBYTE_SELECT_BITS+FIFO_DEPTH_LOG2;
localparam N=( (REQUESTED_SIZE+ACL_BUFFER_ALIGNMENT-1) / ACL_BUFFER_ALIGNMENT) ;
...
wire [(N==1 ? 0 : LOG2N-1):0] in_cache;
assign in_cache = (N==1 ? 1'b0 : reg_i_address[CACHE_ADDRBITS+LOG2N-1:CACHE_ADDRBITS]);

wire [(N==1 ? 0 : LOG2N-1):0] in_cache_unreg;
assign in_cache_unreg = (N==1 ? 1'b0 : i_address[CACHE_ADDRBITS+LOG2N-1:CACHE_ADDRBITS]);
