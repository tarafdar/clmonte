...
module acl_work_group_limiter #(
  parameter integer WG_LIMIT = 1,               // >0
  parameter integer KERNEL_WG_LIMIT = 1,        // >0
  parameter integer MAX_WG_SIZE = 1,            // >0
  parameter integer WG_FIFO_ORDER = 1,          // 0|1
...
(
  ...
  // check for overflow
  localparam MAX_WG_SIZE_WIDTH = ((MAX_WG_SIZE+1) == 0) ? 32 : $clog2(MAX_WG_SIZE+1);
  input logic [MAX_WG_SIZE_WIDTH-1:0] wg_size;

  // Limiter entry
  input logic entry_valid_in;
  input logic [$clog2(KERNEL_WG_LIMIT)-1:0] entry_k_wgid;
  ...
  input logic [$clog2(WG_LIMIT)-1:0] exit_l_wgid;
  ...
  generate
  if( IMPL == "local" && WG_LIMIT >= KERNEL_WG_LIMIT ) begin
