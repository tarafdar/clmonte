...
(
    ...
    output logic [PIPELINE_DEPTH-5:0] valids,
    output logic [SHARINGII-1:0] IIphases,
    input logic inc_pipelined_thread,
    input logic dec_pipelined_thread
);
    (* altera_attribute = "-name auto_shift_register_recognition OFF" *) reg [PIPELINE_DEPTH-5:0] shift_reg;
    ...
    begin
       if (!resetn)
       begin
          shift_reg <= {(PIPELINE_DEPTH-4){1'b0}};
          counter <= {(FIFO_DEPTH_LOG2+1){1'b0}};
          reg_data_in <= 'x;
       end
       else
       begin
          shift_reg <= { shift_reg[PIPELINE_DEPTH-6:0], input_accepted };
    ...
    fifo (
      ...
      .data_in(reg_data_in),
      .data_out(fifo_data),
      .valid_in(shift_reg[PIPELINE_DEPTH-5]),
