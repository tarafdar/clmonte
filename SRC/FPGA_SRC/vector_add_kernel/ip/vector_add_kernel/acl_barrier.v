...
//
// Note this assumes the lowest 32-bits of data is the local_id_3 parameter
// with hardware group id in its upper most bits.
//
//
//===----------------------------------------------------------------------===//
...
input  stall_exit;
input  [WORKGROUP_SIZE_ARG_WIDTH*MAX_SIMULTANEOUS_WORKGROUPS-1:0] num_live_workitems;


reg  [WORKGROUP_SIZE_ARG_WIDTH-1:0] num_current_workitems[MAX_SIMULTANEOUS_WORKGROUPS-1:0];
wire [WORKGROUP_SIZE_ARG_WIDTH-1:0] num_current_workitems_[MAX_SIMULTANEOUS_WORKGROUPS-1:0]; //Create a copy that is a "wire" so that Modelsim will log it automatically
reg  [WORKGROUP_SIZE_ARG_WIDTH-1:0] num_exiting_workitems;
wire valid_in;
wire valid_out;
wire stall_in;
wire stall_out;
...
wire [((LOG2_MAX_SIMULTANEOUS_WORKGROUPS>0) ? LOG2_MAX_SIMULTANEOUS_WORKGROUPS : 1)-1:0] hw_group_entering;
reg  [((LOG2_MAX_SIMULTANEOUS_WORKGROUPS>0) ? LOG2_MAX_SIMULTANEOUS_WORKGROUPS : 1)-1:0] hw_group_exiting;

generate
if (LOG2_MAX_SIMULTANEOUS_WORKGROUPS>0)
begin

  assign hw_group_entering=data_entry[31:32-LOG2_MAX_SIMULTANEOUS_WORKGROUPS];

  always @(posedge clock or negedge resetn)
  begin
    if (~(resetn))
      hw_group_exiting <= {LOG2_MAX_SIMULTANEOUS_WORKGROUPS{1'b0}};
  ...
  assign data_exit = {data_out>>32,
                    hw_group_exiting, 
                    data_out[32-LOG2_MAX_SIMULTANEOUS_WORKGROUPS-1:0]};
end
else
begin
...
defparam barrier_fifo_reorder.WORKGROUP_SIZE_ARG_WIDTH = WORKGROUP_SIZE_ARG_WIDTH;
defparam barrier_fifo_reorder.DEPTH = DEPTH ;
defparam barrier_fifo_reorder.DATA_WIDTH = DATA_WIDTH;
defparam barrier_fifo_reorder.ADDR_WIDTH = 32;
...
genvar i;
  ...
  begin : numcur_gen
    assign live_workitem_count_offset[i] = i*WORKGROUP_SIZE_ARG_WIDTH;
    always @(posedge clock or negedge resetn)
    begin
      if (~(resetn))
      begin
        num_current_workitems[i] <= {WORKGROUP_SIZE_ARG_WIDTH{1'b0}};
      end
      else 
      begin
...
begin
  case (present_state)
    s_IDLE:
      next_state = ((|num_current_workitems[hw_group_exiting]) &&
                    num_current_workitems[hw_group_exiting]==
                      num_live_workitems[ live_workitem_count_offset[hw_group_exiting] +: WORKGROUP_SIZE_ARG_WIDTH]) ?
       s_LOCKED : s_IDLE;
    s_LOCKED:
      next_state = ((num_exiting_workitems == {{(WORKGROUP_SIZE_ARG_WIDTH-1){1'b0}},1'b1}) &&
                    valid_out && ~stall_in) ?
        s_IDLE : s_LOCKED;
  endcase
end
...
begin
  if (~(resetn)) begin
    num_exiting_workitems <= {WORKGROUP_SIZE_ARG_WIDTH{1'b0}};
    valid_exit_pre <= 1'b0;
    stall_in_pre <= 1'b1;
  end
  else begin 
