...
cl_mem simparam_floats_mem_obj;
cl_mem simparam_ints_mem_obj;
cl_mem layerspecs_mem_obj;
cl_mem num_photons_simulated_mem_obj;
cl_mem a_mem_obj;
cl_mem x_mem_obj;
cl_mem A_rz_mem_obj;
...
static void RunGPUi(HostThreadState *hstate)
{
  ...
  InitSimStates(HostMem, hstate->sim, context, command_queue, &num_photons_simulated_mem_obj, 
  ...
  InitDCMem(hstate->sim, context, command_queue, &simparam_floats_mem_obj, &simparam_ints_mem_obj, &layerspecs_mem_obj);
...
//  }
 ...
 // }
  int argn = 0;
  ret = clSetKernelArg(kernel, argn++, sizeof(cl_mem),(void *)&simparam_floats_mem_obj);
  if(ret != CL_SUCCESS){
    printf("Error setting simparam kernel argument, exiting\n");
    exit(-1);
  }
  
  ret = clSetKernelArg(kernel, argn++, sizeof(cl_mem),(void *)&simparam_ints_mem_obj);
  if(ret != CL_SUCCESS){
    printf("Error setting simparam kernel argument, exiting\n");
    exit(-1);
  }
  
  ret = clSetKernelArg(kernel, argn++, sizeof(cl_mem),(void *)&layerspecs_mem_obj);
  if(ret != CL_SUCCESS){
    printf("Error setting layerspecs kernel argument, exiting\n");
    exit(-1);
  }
  
  ret = clSetKernelArg(kernel, argn++, sizeof(cl_mem),(void *)&(num_photons_simulated_mem_obj));
  if(ret != CL_SUCCESS){
    printf("Error setting photons left kernel argument, exiting\n");
    exit(-1);
  }
  
  ret = clSetKernelArg(kernel, argn++, sizeof(cl_mem),(void *)&x_mem_obj);
  if(ret != CL_SUCCESS){
    printf("Error setting x kernel argument, exiting\n");
    exit(-1);
  }
  
  ret = clSetKernelArg(kernel, argn++, sizeof(cl_mem),(void *)&a_mem_obj);
  if(ret != CL_SUCCESS){
    printf("Error setting a kernel argument, exiting\n");
    exit(-1);
  }
  
  ret = clSetKernelArg(kernel, argn++, sizeof(cl_mem),(void *)&Rd_ra_mem_obj);
  if(ret != CL_SUCCESS){
    printf("Error setting Rd_ra kernel argument, exiting\n");
    exit(-1);
  }
  
  ret = clSetKernelArg(kernel, argn++, sizeof(cl_mem),(void *)&A_rz_mem_obj);
  if(ret != CL_SUCCESS){
    printf("Error setting A_rz kernel argument, exiting\n");
    exit(-1);
  }
  
  ret = clSetKernelArg(kernel, argn++, sizeof(cl_mem),(void *)&Tt_ra_mem_obj);
  if(ret != CL_SUCCESS){
 ...
 //   ret = clEnqueueReadBuffer(command_queue, num_photons_simulated_mem_obj, CL_TRUE, 0, sizeof(UINT32), HostMem->n_photons_left, 0, NULL, NULL);
 //   if(ret != CL_SUCCESS){
 ...
 //   printf("num photons left %d after initthreadstate\n", *HostMem->n_photons_left);
  ...
  //{
   // // Run the kernel.
      ...
      if(ret != CL_SUCCESS){
        printf("Error enqueundrange of kernel, exiting %d\n", ret);
        exit(-1);
      }
    ...
    ret = clEnqueueReadBuffer(command_queue, num_photons_simulated_mem_obj, CL_TRUE, 0, sizeof(UINT32), HostMem->n_photons_left, 0, NULL, NULL);
    if(ret != CL_SUCCESS){
      printf("Error reading number of photons left of kernel in loop iteration  exiting %d\n", ret);
      exit(-1);
    }
    printf("[GPU] batch number of photons left %10u\n",  *(HostMem->n_photons_left));
  //}
  ...
  FreeDeviceSimStates(context, command_queue, kernel, program, simparam_floats_mem_obj, simparam_ints_mem_obj, layerspecs_mem_obj,num_photons_simulated_mem_obj, a_mem_obj, x_mem_obj, A_rz_mem_obj, Rd_ra_mem_obj, Tt_ra_mem_obj 
          //photon_x_mem_obj, photon_y_mem_obj, photon_z_mem_obj, photon_ux_mem_obj, 
